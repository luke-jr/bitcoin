...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "getbalance minconf option is only currently supported if dummy is set to \"*\"", self.nodes[0].getbalance, minconf=0)
        assert_equal(self.nodes[0].getbalance("*", 1, True), 50)
        assert_raises_rpc_error(-8, "getbalance minconf option is only currently supported if dummy is set to \"*\"", self.nodes[1].getbalance, minconf=0)
