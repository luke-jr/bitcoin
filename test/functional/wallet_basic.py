...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-3, OUT_OF_RANGE,
            self.nodes[2].sendmany,
            amounts={ address: 10 },
            conf_target=-1,
            estimate_mode='bTc/kB')
        ...
        # Restore to expected balance
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), node_2_bal - self.nodes[2].getbalance())
        self.sync_all(self.nodes[0:3])
        self.nodes[0].generate(1)
        self.sync_all(self.nodes[0:3])
        ...
        if not self.options.descriptors:
            ...
            tx_size = self.get_vsize(self.nodes[2].gettransaction(txid)['hex'])
            import pprint;pprint.pprint(self.nodes[2].getbalance())
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])
            postbalance = self.nodes[2].getbalance()
