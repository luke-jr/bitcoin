...
void Intro::accept()
{
    if ((!m_warning_msg.isEmpty()) && QMessageBox::warning(this, CLIENT_NAME, m_warning_msg + "<br><br>" + tr("Ignore warning?"), QMessageBox::Ignore | QMessageBox::Cancel) == QMessageBox::Cancel) {
        return;
    }
    QDialog::accept();
}
